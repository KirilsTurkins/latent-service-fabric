...
pub(super) fn adversarial(bytes: Vec<u8>) -> Package {
    Package::create(
        bytes,
        manifest(
            "tests/adversarial",
            "tests:adversarial/service@0.1.0",
            &["tests:adversarial/api@0.1.0"],
        ),
        encode_contracts(&[contract(
            "tests:adversarial",
            "api",
            vec![function("value", Vec::new(), Some(ValueType::U32))],
        )]),
        "tests/adversarial",
        "tests:adversarial/api@0.1.0",
        "tests",
...
fn manifest(service: &str, world: &str, exports: &[&str]) -> Value {
...
fn encode_contracts(contracts: &[ContractDescriptor]) -> Vec<u8> {
...
fn contract(package: &str, name: &str, functions: Vec<FunctionDescriptor>) -> ContractDescriptor {
