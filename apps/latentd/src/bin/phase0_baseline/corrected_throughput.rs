...
async fn run_activation_throughput(
    ...
    .await?;
    let saturated_activations = config
        .pool_capacity
...
async fn run_throughput_mode(
    ...
    process_entry: Instant,
    mode: &str,
    activations_per_batch: u32,
    require_queue_saturation: bool,
    samples: &mut Vec<ActivationSample>,
    ...
    for batch in 0..config.throughput_batches {
        ...
        for slot in 0..activations_per_batch {
            let activation_id = ActivationId(format!(
                "baseline-throughput-{mode}-{batch:08}-{slot:04}"
            ));
        ...
        let process_after = observe_process(
            &format!("after_throughput_{mode}_batch_{batch:08}"),
            process_entry,
        );
        ...
        for (activation_id, expected_output, elapsed_micros, phase_timings, outcome) in completed {
            ...
            samples.push(ActivationSample {
                scenario: format!("throughput_{mode}"),
    ...
    if require_queue_saturation
        && (maximum_observed_active_leases != config.pool_capacity
            || maximum_observed_queue_depth != config.pool_queue_capacity)
    {
        return Err(BenchError::new(format!(
            "real activation batch did not reach bounded saturation: active={} expected={}, queue={} expected={}",
            maximum_observed_active_leases,
            config.pool_capacity,
            maximum_observed_queue_depth,
            config.pool_queue_capacity
