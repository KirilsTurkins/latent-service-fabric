...
impl Cache {
    ...
    pub(super) fn read(
        ...
        if asset.size > latent_artifacts::web::MAX_WEB_ASSET_BYTES { return Err(502); }
        let cached = {
            let mut entries = self.entries.try_lock().map_err(|_| 503u16)?;
            entries.iter().position(|(key, _)| key == &digest).map(|index| {
        ...
        let buffer = Arc::new(Buffer { bytes, _charge: permit });
        ...
        if let Ok(mut entries) = self.entries.try_lock() {
            if !entries.iter().any(|(key, _)| key == &digest) {
                if entries.len() == MAX_ENTRIES { entries.pop_front(); }
    ...
    fn reserve(&self, bytes: usize) -> Result<OwnedSemaphorePermit, u16> {
        if bytes > self.maximum_bytes { return Err(503); }
        ...
        for _ in 0..MAX_ENTRIES {
            if let Some(index) = entries.iter().position(|(_, value)| Arc::strong_count(value) == 1) {
...
mod tests {
    ...
    fn asset(bytes: &[u8]) -> WebAsset {
        WebAsset { path: "/a.js".into(), layer: "public/a.js".into(), digest: artifact_blob_digest(bytes).to_string(), size: bytes.len() as u64, media_type: "text/javascript".into() }
    }
    #[test]
    fn eviction_and_clear_do_not_release_pinned_output_bytes() {
        let cache = Cache::new(ENTRY_CHARGE + 3);
        let first = cache.read(&asset(b"one"), |_, out| { out.copy_from_slice(b"one"); Ok(()) }).unwrap();
        assert_eq!(cache.retained_bytes(), ENTRY_CHARGE + 3);
        assert!(cache.read(&asset(b"two"), |_, _| panic!("must reserve before read")).is_err());
        cache.clear();
        assert_eq!(cache.retained_bytes(), ENTRY_CHARGE + 3);
        drop(first);
        assert_eq!(cache.retained_bytes(), 0);
        let next = cache.read(&asset(b"two"), |_, out| { out.copy_from_slice(b"two"); Ok(()) }).unwrap();
    ...
    fn corruption_refetches_once_and_bad_source_never_becomes_a_cache_hit() {
        ...
        drop(cache.read(&descriptor, |_, out| { out.copy_from_slice(b"abc"); Ok(()) }).unwrap());
        assert!(cache.read(&descriptor, |_, _| panic!("verified hit")).is_ok());
        {
            let mut entries = cache.entries.lock().unwrap();
            Arc::get_mut(&mut entries[0].1).unwrap().bytes[0] = b'x';
        }
        let repaired = cache.read(&descriptor, |_, out| { out.copy_from_slice(b"abc"); Ok(()) }).unwrap();
        assert_eq!(repaired.bytes, b"abc");
        assert_eq!(cache.corruptions.load(Ordering::Relaxed), 1);
        drop(repaired);
        cache.clear();
        assert!(cache.read(&descriptor, |_, out| { out.copy_from_slice(b"bad"); Ok(()) }).is_err());
    ...
    fn zero_length_assets_are_verified_and_oversized_declarations_do_not_read() {
        let cache = Cache::new(4096);
        assert!(cache.read(&asset(b""), |_, out| { assert!(out.is_empty()); Ok(()) }).is_ok());
