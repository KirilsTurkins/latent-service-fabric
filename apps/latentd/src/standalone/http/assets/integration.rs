...
use crate::{config::NodeConfig, standalone::http::{tests::fixture as node_fixture, HttpOwner, HttpServices}};
use latent_artifacts::{DirectoryArtifactRepository, DirectoryArtifactRepositoryConfig, AdmissionStorageLimits, ReleaseLifecycleAction, ReleaseLifecycleReason};
use latent_core::TenantId;
use std::{sync::{atomic::{AtomicBool, Ordering}, Arc}, time::Duration};
use tempfile::TempDir;
use tokio::{io::{AsyncReadExt, AsyncWriteExt}, net::TcpStream, time::Instant};
...
impl Harness {
    async fn new() -> Self {
        ...
        let value = node_fixture::config(&root);
        let settings = serde_json::from_value::<NodeConfig>(value.clone()).unwrap().derive().unwrap();
        ...
        assert!(node.node.http_snapshot().unwrap().assets.is_some(), "production startup installs one shared asset owner");
        let storage = TempDir::new().unwrap();
        let current = Arc::new(AtomicBool::new(true));
        let repository = Arc::new(DirectoryArtifactRepository::open_enforced(
            storage.path().join("catalog"), DirectoryArtifactRepositoryConfig::default(),
            AdmissionStorageLimits::default(), Arc::new(fixture::Authority(Arc::clone(&current))),
        ).unwrap());
        let owner = HttpOwner::start(settings.http.unwrap(), HttpServices {
            manager: node.node.manager.clone(), deployments: node.deployments.clone(),
            cleanup: node.node.cleanup.as_ref().unwrap().handle(), clock: node.node.clock.clone(),
        ...
        Self { owner, node, repository, current, _storage: storage }
    }
    ...
    fn publish(&self, operation: &str, bytes: &[u8]) -> String {
        ...
        self.repository.select_web_publication(&reference).unwrap().asset_url("/index.html").unwrap()
    }
    async fn call(&self, method: &str, path: &str, extra: &str, token: &str) -> (u16, String, Vec<u8>) {
        ...
        tokio::time::timeout(Duration::from_secs(8), socket.take(9 * 1024 * 1024).read_to_end(&mut bytes)).await.unwrap().unwrap();
    ...
    async fn finish(self) {
        node_fixture::wait(|| {
            ...
            snapshot.connections == 0 && snapshot.exchanges == 0 && self.store().snapshot().active_reads == 0
        }).await;
        ...
        self.owner.shutdown(Instant::now() + Duration::from_secs(5)).await.unwrap();
        assert!(handle.snapshot().clean());
        self.node.shutdown().await;
    }
}
fn etag(headers: &str) -> &str {
    headers.lines().find_map(|line| line.strip_prefix("ETag: ")).unwrap()
}
...
async fn real_get_head_conditionals_and_atomic_release_replacement_never_need_a_renderer() {
    ...
    for method in ["GET", "HEAD"] {
        let (status, header, body) = h.call(method, &first, &condition, node_fixture::TOKEN).await;
    ...
    let (code, header, body) = h.call("GET", &second, &condition, node_fixture::TOKEN).await;
    assert_eq!(code, 200);
    assert_ne!(etag(&header), first_tag);
    assert_eq!(body, b"<h1>two</h1>");
    assert_eq!(h.call("GET", &first, "", node_fixture::TOKEN).await.2, b"<h1>one</h1>");
    let (code, _, body) = h.call("GET", &second, &format!("If-Match: {first_tag}\r\nIf-None-Match: *\r\n"), node_fixture::TOKEN).await;
    assert_eq!(code, 412);
    assert!(body.is_empty());
...
async fn errors_private_layers_methods_ranges_compression_and_tenants_remain_fail_closed() {
    ...
    for suffix in ["/metadata/private.json", "/metadata/web-application.json", "/server/renderer.wasm", "/public/index.html", "/missing.js"] {
        let (code, headers, body) = h.call("HEAD", &format!("{base}{suffix}"), "", node_fixture::TOKEN).await;
        assert_eq!(code, 404);
    ...
    assert_eq!(h.call("GET", &path, "Accept-Encoding: gzip, identity;q=0\r\n", node_fixture::TOKEN).await.0, 406);
    let (code, headers, body) = h.call("GET", &path, "Range: bytes=0-0,2-3\r\nAccept-Encoding: gzip, br\r\n", node_fixture::TOKEN).await;
    assert_eq!(code, 200);
    assert_eq!(body, b"hello");
    assert!(headers.contains("Accept-Ranges: none"));
    assert!(!headers.contains("Content-Encoding:"));
    assert!(matches!(h.call("GET", &path, "", node_fixture::OTHER).await.0, 403 | 404));
    for suffix in ["/%2findex.html", "/%2e%2e/index.html", "/../index.html", "/%69ndex.html", "/index.html?other=1"] {
        assert_eq!(h.call("GET", &format!("{base}{suffix}"), "", node_fixture::TOKEN).await.0, 400);
    }
    let (_, script_headers, _) = h.call("GET", &format!("{base}/app.js"), "", node_fixture::TOKEN).await;
...
async fn cached_bytes_and_prepared_304_are_not_authority_after_policy_change_or_revocation() {
    ...
    drop(prepared);
    assert_eq!(h.call("HEAD", &path, "If-None-Match: *\r\n", node_fixture::TOKEN).await.0, 403);
    h.current.store(true, Ordering::Release);
    let reference = fixture::publish(&h.repository, "same-package", b"hello");
    h.repository.transition_web_publication(fixture::context("revoke", 1), &reference, ReleaseLifecycleAction::Revoke, ReleaseLifecycleReason::OperatorRevocation, &mut |_| Ok(())).unwrap();
    assert_eq!(h.call("GET", &path, "If-None-Match: *\r\n", node_fixture::TOKEN).await.0, 403);
    assert!(store.snapshot().retained_buffer_bytes > 0, "cache bytes do not imply permission");
    h.finish().await;
}
...
async fn physical_corruption_symlinks_and_atomic_directory_substitution_cannot_change_served_bytes() {
    ...
    assert_eq!(h.call("HEAD", &path, "If-None-Match: *\r\n", node_fixture::TOKEN).await.0, 502);
    assert_eq!(h.store().snapshot().cache_entries, 0);
    std::fs::write(&file, b"hello").unwrap();
    let outside = h._storage.path().join("outside");
    ...
    std::fs::rename(&root, h._storage.path().join("original-catalog")).unwrap();
    ...
    assert_eq!(h.call("GET", &path, "", node_fixture::TOKEN).await.2, b"hello");
    h.finish().await;
}
...
async fn disconnected_blocking_read_keeps_its_slot_until_actual_completion() {
    ...
    *store.pause.lock().unwrap() = Some((entered, blocked));
    ...
    socket.write_all(node_fixture::request("GET", &path, node_fixture::TOKEN, 0, true).as_bytes()).await.unwrap();
    started.recv_timeout(Duration::from_secs(5)).unwrap();
    assert_eq!(store.snapshot().active_reads, 1);
    drop(socket);
    node_fixture::wait(|| h.owner.handle().snapshot().connections == 0).await;
    assert!(!store.shutdown(Instant::now() + Duration::from_millis(10)).await);
    assert_eq!(store.snapshot().active_reads, 1);
    resume.send(()).unwrap();
