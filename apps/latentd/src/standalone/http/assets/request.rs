...
impl Request {
    ...
    pub(super) fn parse(bytes: &[u8], tenant: &TenantId) -> Result<Self, u16> {
        ...
        let matching = single(message.headers, "if-match")?.map(Tags::parse).transpose()?;
        let none_matching = single(message.headers, "if-none-match")?.map(Tags::parse).transpose()?;
        if !identity_allowed(single(message.headers, "accept-encoding")?)? {
            return Err(406);
        }
        Ok(Self { reference, path, head, matching, none_matching })
    }

    pub(super) fn status(&self, etag: &str) -> Result<u16, u16> {
        // RFC 9110 precedence: If-Match is evaluated before If-None-Match.
        if self.matching.as_ref().is_some_and(|tags| !tags.matches(etag, false)) {
            return Err(412);
        }
        Ok(if self.none_matching.as_ref().is_some_and(|tags| tags.matches(etag, true)) {
            304
        } else {
            200
        })
...
fn locator(target: &str, tenant: &TenantId) -> Result<(PublicationRef, String), u16> {
    ...
    latent_artifacts::package::validate_package_path(
        path, latent_artifacts::package::PackageLimits::default(),
    ).map_err(|_| 400u16)?;
...
fn single<'a>(headers: &'a [httparse::Header<'_>], name: &str) -> Result<Option<&'a str>, u16> {
    let mut values = headers.iter().filter(|header| header.name.eq_ignore_ascii_case(name));
...
impl Tags {
    fn parse(value: &str) -> Result<Self, u16> {
        ...
        loop {
            ...
            if weak { remaining = &remaining[2..]; }
            let tail = remaining.strip_prefix('"').ok_or(400u16)?;
            let end = tail.find('"').ok_or(400u16)?;
            if end > 128 || !tail[..end].bytes().all(|b| b == 0x21 || (0x23..=0x7e).contains(&b)) {
                return Err(400);
            }
            tags.push((weak, remaining[..end + 2].to_owned()));
            remaining = tail[end + 1..].trim_matches([' ', '\t']);
            if remaining.is_empty() { break; }
            remaining = remaining.strip_prefix(',').ok_or(400u16)?.trim_start_matches([' ', '\t']);
        }
        Ok(Self::Values(tags))
    }
    fn matches(&self, etag: &str, allow_weak: bool) -> bool {
        match self {
            Self::Any => true,
            Self::Values(tags) => tags.iter().any(|(weak, value)| (allow_weak || !weak) && value == etag),
        }
    }
}
...
fn identity_allowed(value: Option<&str>) -> Result<bool, u16> {
    ...
    if value.trim().is_empty() { return Ok(true); }
    let mut identity = None;
    let mut wildcard = None;
    for (index, entry) in value.split(',').enumerate() {
        if index >= 16 { return Err(431); }
        let mut parts = entry.trim().split(';');
        let coding = parts.next().ok_or(400u16)?.trim();
        if coding.is_empty() || !coding.bytes().all(|b| b.is_ascii_alphanumeric() || b"!#$%&'*+-.^_`|~".contains(&b)) {
            return Err(400);
        }
        let quality = parts.next().map_or(Ok(1000), |part| {
            let (name, value) = part.trim().split_once('=').ok_or(400u16)?;
            if !name.trim().eq_ignore_ascii_case("q") { return Err(400); }
            qvalue(value.trim())
        })?;
        if parts.next().is_some() { return Err(400); }
        ...
        if let Some(slot) = slot {
            if slot.replace(quality).is_some() { return Err(400); }
        }
    }
...
fn qvalue(value: &str) -> Result<u16, u16> {
    ...
    if whole == "1" {
        return if fraction.bytes().all(|b| b == b'0') { Ok(1000) } else { Err(400) };
    }
    let mut result = 0;
    for index in 0..3 {
        result = result * 10 + u16::from(fraction.as_bytes().get(index).copied().unwrap_or(b'0') - b'0');
    }
    Ok(result)
}
...
mod tests {
    ...
    fn immutable_locator_rejects_aliases_and_private_or_noncanonical_paths() {
        ...
        let prefix = format!("{IMMUTABLE_ASSET_PREFIX}publication:sha256:{}/", "ab".repeat(32));
        assert!(locator(&format!("{prefix}main.js"), &tenant).is_ok());
        for suffix in ["", "../main.js", "a/../main.js", "a//main.js", "./main.js", "main.js?x=1", "%6dain.js", "%2fmain.js", "a\\main.js", "main.js#x"] {
            assert!(locator(&format!("{prefix}{suffix}"), &tenant).is_err(), "{suffix}");
        }
    }
    #[test]
    fn conditional_lists_support_weak_comparison_commas_and_strong_precedence() {
        ...
        for invalid in ["", "*, \"x\"", "\"x\",", "w/\"x\"", "unquoted", "\"x\" garbage"] {
            assert!(Tags::parse(invalid).is_err(), "{invalid}");
        }
    }
    #[test]
    fn identity_exclusion_is_honored_without_decompression_or_qvalue_guessing() {
        for value in [None, Some(""), Some("gzip, br"), Some("identity;q=0.001"), Some("*;q=0, identity;q=1")] {
        ...
        for value in ["identity;q=1.1", "identity;q=-1", "identity;q=0.0001", "identity;q=NaN", "identity;q=0, identity;q=1", "gzip,,br"] {
