...
fn topology(pool: &FixedCellPool) -> Topology {
    let tasks: Vec<_> = fs::read_dir("/proc/self/task").expect("task directory")
        .map(|entry| entry.expect("task entry").path()).collect();
    ...
    for entry in fs::read_dir("/proc/self/fd").expect("file descriptors") {
        ...
        if let Some(inode) = target.strip_prefix("socket:[").and_then(|value| value.strip_suffix(']')) {
...
fn synthetic_release(template: &CapsuleManifest, index: u32) -> CapsuleArtifact {
    ...
    let mut component_bytes = vec![0, 97, 115, 109, 13, 0, 1, 0, 0, 9, 4, b's', b'e', b'e', b'd'];
...
fn verify_catalog(repo: &dyn ArtifactRepository, template: &CapsuleManifest) {
    ...
    loop {
        ...
        for descriptor in &page.entries {
            if let Some(previous) = &after {
                assert!(&descriptor.release_digest > previous, "strict ordering with no duplicates");
            }
            let index: u32 = descriptor.reference.0.strip_prefix("local://scale/")
                .expect("scale reference").parse().expect("scale index");
            assert!(index < RELEASE_COUNT);
            let expected = synthetic_release(template, index);
            assert_eq!(descriptor, &expected.descriptor);
            assert_eq!(block_on(repo.resolve(&ArtifactQuery {
            ...
            })).expect("resolve published release"), Some(descriptor.clone()));
            assert_eq!(block_on(repo.fetch(&descriptor.release_digest)).expect("byte-for-byte fetch"), expected);
            after = Some(descriptor.release_digest.clone());
            seen += 1;
        }
        match page.next_after {
            Some(cursor) => {
                assert_eq!(Some(&cursor), last.as_ref(), "continuation must match last entry");
    ...
    assert_eq!(seen, RELEASE_COUNT, "all persisted releases must be retrievable");
}
...
fn catalog_scale_child() {
    ...
    let pool = FixedCellPool::new(FixedCellPoolConfig::new(
        NodeId("catalog-scale-node".to_owned()), CellClass::Standard, 2, 2,
    ...
    let repo = DirectoryArtifactRepository::open(root, DirectoryArtifactRepositoryConfig {
    ...
    assert_eq!(topology(&pool), opened, "opening adds only a fixed ownership FD");
    ...
    if mode == "publish" {
        for index in 0..RELEASE_COUNT {
            ...
            assert_eq!(block_on(repository.publish(release)).expect("durable publication"), descriptor);
            if (index + 1) % 10_000 == 0 {
                assert_eq!(topology(&pool), opened, "registration checkpoint {}", index + 1);
                eprintln!("published {} complete releases through ArtifactRepository::publish", index + 1);
            }
        }
    }
    verify_catalog(repository, &template);
    let after = topology(&pool);
    assert_eq!(after, opened, "no per-release execution or OS resource growth");
    ...
    drop(repo);
    assert_eq!(topology(&pool), baseline, "dropping repository releases its ownership FD");
}
...
fn run_child(root: &Path, mode: &str, log_path: &Path) {
    ...
    let mut child = Command::new(std::env::current_exe().expect("acceptance test binary"))
        .args(["--exact", "catalog_scale_child", "--ignored", "--nocapture", "--test-threads=1"])
        .env(ROOT_ENV, root)
        .env(MODE_ENV, mode)
        .stdout(Stdio::from(log.try_clone().expect("clone log")))
        .stderr(Stdio::from(log))
        .spawn().expect("spawn isolated catalog process");
...
fn production_catalog_100k() {
    ...
    assert_eq!(fs::read_dir(root.path().join("releases")).expect("completed directories").count(), RELEASE_COUNT as usize);
