...
For the packaged C workflow, start with [application development](../start/application-development.md) and select `c` when [getting the tools](../start/developer-setup.md). The controller builds your project on Linux or WSL, creates node credentials, and runs the real-node tests. You do not need to build LSF or install the language compiler on Windows.
