...
The node uses fixed in-process cell pools, fresh Wasmtime stores and explicit
reuse or quarantine through the [scheduler](../scheduling.md) and
[activation lifecycle](../activation-lifecycle.md). Optional isolated native
compilation uses temporary bounded workers, not guest execution cells.
Asynchronous capabilities and local child calls keep their activation resources
through waits. Separate guest execution processes and state transactions remain
unsupported.
...
clarifies ownership across asynchronous providers and descendant calls.
...
can block on a descendant, the [local-call provider](../runtime/local-service-invocation.md) must either
...
or evict an active guest stack. Reuse still requires affirmative
...
A guest trap must terminate only that activation. The [capability broker](../runtime/capability-broker.md)
keeps handles in an activation-owned table. The current `local-experimental-v1` profile trusts the standalone
...
protected configuration and isolated compilation of the supported
...
For asynchronous work, "host-call ownership is released" means that
