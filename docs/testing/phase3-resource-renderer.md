...
Every JSON has a SHA-256 sidecar. The receipt preserves its original pending ticket status. The separate [OCI campaign](phase3-resource-oci.md) now supplies token/resolver/redirect measurements at two ceilings. The [provider checkpoint](phase3-resource-events.md) and [standalone provider profiles](phase3-resource-recovery.md) separately establish their measured resource populations. Actual browser journeys are retained by the Angular reference qualification.
