...
Exact publication selection follows the [publication runtime contract](reference/publication-runtime.md). The component digest remains executable identity; deployment revisions retain a separate tenant-scoped publication.
...
Existing formats 1–4 are accepted only after checksum, manifest, history and
reconstructed-route validation. Version 1 lacks object versions, so every live
deployment receives the persisted catalog generation as its initial stamp.
Startup recovers exact publication associations and durably upgrades to format 5
before exposing routes. It preserves legacy manifests, deterministic revision IDs,
object versions, combined transaction versions and receipt bytes. Ambiguous legacy
association without an immutable artifact migration mapping is rejected. An empty
legacy generation-zero catalog remains empty. Later writers preserve all histories
through the same atomic publication protocol.
...
Updates still visit every deployment, verify every distinct referenced release, and persist a complete bounded catalog. Unchanged applies retain this full-catalog verification and write cost. Use atomic batches rather than repeated single-deployment updates for bulk changes. The compiler sorts borrowed deployment references by `(release digest, deployment ID)` and processes one release group at a time. Each release is fetched exactly once per compilation, including unchanged applies. Its full artifact metadata is dropped **before** fetching the next release; there is no aggregate full-artifact cache. Component bytes are dropped after verification and are never retained in routing snapshots or prepared for execution.

An immutable generation may retain an optional, bounded memo of complete verified metadata fingerprints. It retains no artifact payload or previous catalog owner. Matching freshly fetched metadata, unchanged deployment contents and the same compiler configuration allow reuse of canonical deployment, revision and export derivations. Release identity or a descriptor's claimed schema digest alone cannot authorize this reuse. The fingerprint attempt is limited to 512 KiB and depth 64; metadata outside those bounds follows the full derivation path. Only entries fitting the remaining compilation metadata budget are retained, so missing memo space reduces reuse without adding an admission failure. Validation, namespace/schema consistency, policy and index charges still run for the complete desired state. An accepted unchanged apply still advances its object and catalog stamps.

Unchanged immutable records can be shared between generations. Route indexes for affected `(tenant, service)` scopes are rebuilt; eligible unaffected packed scopes are copied only after checking and remapping their record positions. Reweighting changes those indexes even though it preserves the logical revision ID. Page indexes and encoded row sizes are rebuilt for the new generation, including object-stamp digit changes. Full-catalog scans, generation-local arrays and complete serialization remain; retained old pins keep their own coherent generation until dropped.

When fresh schema derivation is needed, canonical contract trees and encoded bytes exist only while fingerprinting one contract of the current release. The per-release fingerprint cache and cross-release tenant/service compatibility map retain computed SHA-256 fingerprints, not canonical trees, documentation or type descriptions. The artifact-verification working set remains bounded by one release's metadata and its canonicalization scratch space; compiled records, indexes and the final encoded catalog are separate retained or temporary owners. With the production artifact repository, per-release metadata reads retain their configured bound (4 MiB by default); custom artifact repositories must also bound individual fetch results. These bounds do not limit the number of concurrent management calls or explicitly pinned old snapshots; their scheduling and lifetimes remain the embedding node's responsibility.

Grouping and reuse are internal compilation choices. Published services and revision candidates keep their deterministic ordering. Canonical schema fingerprints, revision IDs, weighted selection, persisted bytes and checksum rules remain unchanged.
...
reopen. They do not require a large catalog or execution workload. Standalone
node integration tests also exercise deployment application through the generated
