...
| Native distribution #308 | [Nonpublishing foundation run 35763422270](https://github.com/KirilsTurkins/latent-service-fabric/actions/runs/35763422270) passed both native VM profiles and remains historical evidence. The [current foundation plan](development/native-release-gate.md#current-format-native-foundation) replaces its obsolete-format predecessor declaration with a distinct rc.2 source. | Qualify the real current-format foundation, bind its observed archive identity, test the genuine compatible-version pair and publish the authenticated bundle through the protected release owner. |
