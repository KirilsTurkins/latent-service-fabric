...
func mustPanic(t *testing.T, f func()) {
	t.Helper()
	defer func() { if recover() == nil { t.Fatal("ownership violation accepted") } }()
	f()
}
...
func TestAliasesShareExactlyOneDestructor(t *testing.T) {
	...
	alias := o
	calls := 0
	drop := func(value int) { calls++; if value != 42 { t.Fatal(value) } }
	o.Close(drop)
	alias.Close(drop)
...
func TestPendingBorrowPreventsCloseMoveAndConcurrentBorrow(t *testing.T) {
	...
	release()
	mustPanic(t, release)
	if o.Take() != 42 { t.Fatal("borrow corrupted value") }
}
...
func TestDeferredBorrowReleaseSurvivesDeclaredErrorAndPanic(t *testing.T) {
	...
	use := func(fail bool) bool {
		_, release := o.Borrow()
		defer release()
		if fail { panic("application failure") }
		return false
	}
	if use(false) { t.Fatal("declared error changed") }
	mustPanic(t, func() { use(true) })
	if o.Take() != 42 { t.Fatal("borrow leaked") }
}
...
func TestOldReleaseCannotRefundAnotherBorrow(t *testing.T) {
	...
	mustPanic(t, func() { o.Take() })
	second()
	if o.Take() != 42 { t.Fatal("wrong owner released") }
}
