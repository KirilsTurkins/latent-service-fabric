...
func Run(which uint32, url string, _ uint64) uint64 {
	...
	if which == 0 { method = http.MethodGet }
	if which == 1 { method = http.MethodHead }
	...
	if r.IsOk() { return uint64(r.Ok().Status) + 1000 * uint64(len(r.Ok().Body)) }
	switch r.Err().Tag() {
	case http.HttpErrorPermissionDenied: return 10
	case http.HttpErrorUncertain: return 11
	default: panic("unexpected HTTP outcome")
	}
}
