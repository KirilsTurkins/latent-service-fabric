...
func Run(_ uint32, topic string, handle uint64) uint64 {
	...
	if r.IsOk() {
		value := r.Ok()
		if value.EventId == "" || value.StreamName == "" { panic("incomplete publication receipt") }
		return value.Sequence
	}
	switch r.Err().Tag() {
	case events.EventErrorPermissionDenied: return 10
	case events.EventErrorUncertain: return 11 // preserve uncertainty; never retry
	default: panic("unexpected event outcome")
	}
}
