...
func Run(which uint32, _ string, _ uint64) uint64 {
	function := "spin"
	if which == 0 { function = "answer" }
	if which == 1 { function = "fail" }
	...
	case service.InvocationOutcomeSuccess:
		if string(outcome.Success().Payload) != "[42]" { panic("child output changed") }; return 42
	case service.InvocationOutcomeDeclaredError:
		if len(outcome.DeclaredError().Payload) == 0 { panic("missing declared error") }; return 10
	case service.InvocationOutcomePlatformFailure:
		switch outcome.PlatformFailure().Code {
		case service.PlatformErrorCodePermissionDenied: return 11
		case service.PlatformErrorCodeCancelled: return 12
		case service.PlatformErrorCodeDeadlineExceeded: return 13
		case service.PlatformErrorCodeResourceExhausted: return 14
		}
	}
