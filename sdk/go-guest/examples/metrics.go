...
func Run(which uint32, name string, _ uint64) uint64 {
	...
	if r.IsOk() { if r.Ok() { return 1 }; return 0 }
	switch r.Err().Tag() {
	case metrics.TelemetryErrorInvalidName: return 10
	case metrics.TelemetryErrorBudgetExhausted: return 11
	case metrics.TelemetryErrorUnavailable: return 12
	default: panic("unknown metric error")
	}
}
