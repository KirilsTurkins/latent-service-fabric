//! Closed WASI support for the pinned NativeAOT library profile.
//! This component never imports host authority or owns an OS resource.
...
impl clocks::monotonic_clock::Guest for ClosedRuntime {
    fn now() -> u64 {
        panic!("WASI clocks are unsupported; import latent:clock/monotonic explicitly");
    }
...
impl filesystem::types::GuestDescriptor for ClosedDescriptor {
    fn read_via_stream(&self, _offset: u64) -> Result<io::streams::InputStream, filesystem::types::ErrorCode> {
        Err(filesystem::types::ErrorCode::NotPermitted)
    }
    fn write_via_stream(&self, _offset: u64) -> Result<io::streams::OutputStream, filesystem::types::ErrorCode> {
        Err(filesystem::types::ErrorCode::NotPermitted)
    }
