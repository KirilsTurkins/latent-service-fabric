...
from tools.dotnet_guest.project import create, declare_runtime
...
def project(directory: Path, name: str) -> Path:
    ...
    (directory / "wit/world.wit").write_text(declare_runtime(read_file(source / "world.wit").decode()), encoding="utf-8")
    (directory / "src/Main.cs").write_bytes(read_file(ROOT / "sdk/dotnet-guest/examples" / (name + ".cs")))
    ...
    value["limits"].update(cpuFuel=10_000_000_000, childCalls=16 if name == "service" else 0,
        memoryBytes=(256 if name == "service" else 128) * 1024 * 1024,
...
def compile_all(output: Path, tools: Path, names=NAMES):
    ...
    report = {"language": "dotnet", "status": "failed", "runtimeQualified": False, "builds": []}
    try:
        (output / "projects").mkdir()
        for name in names:
            source = project(output / "projects" / name, name)
            build(source, output / ("dotnet-" + name), target / ("debug/examples/capsule_contracts" + suffix),
