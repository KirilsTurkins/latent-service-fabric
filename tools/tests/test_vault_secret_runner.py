"""Select the existing Cargo harness and confine disposable Vault cleanup."""
...
SCRIPT = Path(__file__).resolve().parents[1] / "run_vault_secret_tests.py"
SPEC = importlib.util.spec_from_file_location("vault_runner", SCRIPT)
RUNNER = importlib.util.module_from_spec(SPEC)
with patch.object(sys, "path", [str(SCRIPT.parent), *sys.path]):
    SPEC.loader.exec_module(RUNNER)


class VaultRunnerTests(unittest.TestCase):
    def test_reuses_only_the_named_harness_inside_the_cargo_target(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            target = root / "target"
            target.mkdir()
            executable = target / "vault_secrets"
            executable.touch()
            manifest = root / "cargo.jsonl"
            artifact = {"reason": "compiler-artifact", "target": {"name": "vault_secrets"},
    ...
    def test_missing_or_ambiguous_cargo_artifacts_are_rejected(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for items in [[], [{"reason": "compiler-artifact", "target": {"name": "vault_secrets"},
