"""Adjacent mutation/reopen owners with a separate bounded allocation stage."""
import os
...
from tools.artifact_identity_evidence.runs import probe_resources
...
from tools.optimization_evidence.resources import clean
...
def _child(selection, args, repo, build, suite, output, root, group, marker_ref, deadline, save,
    ...
    profiled = model.profiled(selection["mode"])
    if profiled:
    ...
    env.pop("LSF_CATALOG_MUTATION_REOPEN_RECEIPT", None)
    if reopen_ref is not None:
        env["LSF_CATALOG_MUTATION_REOPEN_RECEIPT"] = str(output / reopen_ref["path"])
    ...
    try:
        ...
        if (raw_dir / "catalog-mutations.json").is_file():
            row["raw"] = reference(raw_dir / "catalog-mutations.json", output)
        if not profiled:
        ...
        save()
    if model.is_initial(selection["mode"]):
        require(row["raw"] is not None and row["process"] is not None, "catalog-initial-receipt-missing")
        require(clean(row["process"]), "catalog-mutation-initial-wrapper-not-clean")
        if profiled:
            # The logical owner remains allocation/reopen. Both run through the
            # shared allocation wrapper; bind the actual source child separately.
            probe_resources({**row, "mode": "allocation"}, binary, suite["tools"])
        source_process = row["probe_process"] if profiled else row["process"]
        receipt = data.post_exit(root, read_json(output / marker_ref["path"]), source_process, row["raw"])
...
def _group(selections, args, repo, build, suite, output, target, deadline, save):
    ...
    try:
        ...
        write(group / "filesystem-reserve.json", data.reserve(root, args.profile == "full" and not model.profiled(first["mode"])))
        reopen_ref = None
...
def execute(args, repo):
    ...
    try:
        ...
        ordinary = [row for row in rows if not model.profiled(row["mode"])]
        ...
        profiled = rows[len(ordinary):]
        for offset in range(0, len(profiled), 2):
            _group(profiled[offset:offset + 2], args, repo, build, suite, output, target, allocation_deadline, save)
