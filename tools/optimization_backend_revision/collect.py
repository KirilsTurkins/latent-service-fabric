...
def execute(args, repo):
    ...
    deadline = began + (9000 if args.profile == "full" else 300) * 10**9
    suite = {"schema": "latent.optimization.backend-revision-suite.v1", "profile": args.profile,
             "plan": model.plan(args.profile), "builds": legacy.ref(build_path, output),
    ...
    try:
        for repetition, variant in model.population(args.profile):
            ...
            selected = model.plan(args.profile, repetition)
            ...
            try:
                with tempfile.TemporaryDirectory(prefix="backend-revision-data-owned-", dir=target) as data:
                    ...
                    command(argv, current / "collector.log", int(selected["maximum_run_seconds"]), repo, deadline, env,
                ...
                for name, filename in (("raw", "candidate.json"), ("process", "collector.log.process.json"),
