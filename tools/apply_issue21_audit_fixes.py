#!/usr/bin/env python3
"""Apply the reviewed Issue #21 corrections before CI validation."""
...
replace_once(
    ...
    '| BLAKE3 | 1.8.5 | General content hashing selected for runtime cache and preparation keys |\n'
...
replace_once(
    "tools/toolchain-smoke/Cargo.toml",
    '[[example]]\nname = "echo-capsule"\npath = "examples/echo_capsule/component.rs"\ncrate-type = ["cdylib"]\ntest = false\nbench = false\ndoc = false\n',
    '[[example]]\nname = "echo-capsule"\npath = "examples/echo_capsule/component.rs"\ncrate-type = ["cdylib"]\ntest = false\nbench = false\ndoc = false\n\n'
    '[[example]]\nname = "oversized-log-capsule"\npath = "examples/oversized_log_capsule/component.rs"\ncrate-type = ["cdylib"]\ntest = false\nbench = false\ndoc = false\n',
)
...
replace_once(
    "crates/latent-wasmtime/src/backend.rs",
...
replace_once(
    ...
    'const MAX_LOG_FIELD_VALUE_BYTES: usize = 256;\n'
    'const HOSTCALL_FUEL_FIXED_OVERHEAD_BYTES: usize = 256;\n'
...
'''
replace_once("crates/latent-wasmtime/src/host.rs", old_limiter, new_limiter)
...
host_path = ROOT / "crates/latent-wasmtime/src/host.rs"
host_text = host_path.read_text(encoding="utf-8")
if "aggregate_memory_budget_counts_all_linear_memories" in host_text:
    raise RuntimeError("host limiter tests already exist")
host_text += r'''
...
mod tests {
    ...
    fn aggregate_memory_budget_counts_all_linear_memories() {
        ...
        assert_eq!(limiter.peak_memory_bytes(), (2 * WASM_PAGE_BYTES) as u64);

        let error = limiter
            .memory_growing(WASM_PAGE_BYTES, 2 * WASM_PAGE_BYTES, None)
            .expect_err("aggregate growth beyond the activation budget must trap");
        assert!(error.to_string().contains("aggregate linear-memory budget exceeded"));
        assert_eq!(limiter.current_memory_bytes(), 2 * WASM_PAGE_BYTES);
        assert_eq!(limiter.peak_memory_bytes(), (2 * WASM_PAGE_BYTES) as u64);
    }
...
#[allow(dead_code)]
fn _assert_paths_are_files(paths: &[&Path]) {
    for path in paths {
        assert!(path.is_file());
    }
}
...
replace_once(
    ...
    'The store is initialized with the invocation fuel grant, an epoch deadline, and the effective minimum of the invocation, cell, and node memory limits. The generated typed binding calls `echo`; no handwritten canonical-ABI value decoding is used.',
    'The store is initialized with the invocation fuel grant, an epoch deadline, and the effective minimum of the invocation, cell, and node memory limits. Linear-memory accounting is aggregate across every memory in the store, so a multi-memory component cannot multiply the activation budget; reported peak memory is the aggregate peak. Each fresh store also receives Component Model host-call fuel derived from the delegated log budget and the maximum permitted log message/field shapes, preventing oversized canonical-ABI lifting before the host log checks run. The generated typed binding calls `echo`; no handwritten canonical-ABI value decoding is used.',
)
replace_once(
    "docs/development/phase-0-wasmtime.md",
    'The repository contract gate builds the echo component and invokes it through the backend:',
    'The repository contract gate builds the echo component, loads both its generated `capsule.json` and Wasm bytes, invokes it through the backend, and builds a same-interface oversized-log fixture to verify the host-call allocation boundary:',
)
replace_once(
    "docs/development/phase-0-wasmtime.md",
...
print("applied Issue #21 audit corrections")
