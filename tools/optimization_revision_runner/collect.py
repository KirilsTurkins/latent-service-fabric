...
def execute(args, repo: Path) -> int:
    ...
    from . import budget, codec, engine, ownership, recovery
    from tools.optimization_revision_evidence import budget_builds, codec_builds, engine_builds, ownership_builds, recovery_builds
    experiment = getattr(args, "experiment", "warm")
    if experiment not in ("warm", "budget", "recovery", "ownership", "codec", "engine"):
        raise ValueError("unknown-revision-experiment")
    selected_model = {"budget": budget, "recovery": recovery, "ownership": ownership, "codec": codec, "engine": engine}.get(experiment)
    build_api = {"recovery": recovery_builds, "ownership": ownership_builds, "codec": codec_builds, "engine": engine_builds}.get(experiment, budget_builds)
