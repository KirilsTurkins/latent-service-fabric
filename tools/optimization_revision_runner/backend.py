...
def build_backend(root: Path, target: Path, label: str, output: Path, deadline: int) -> dict:
    ...
    before = source(root)
    retained = inputs(root, label, output)
    ...
    binary = legacy.retain(matches[0], output, f"builds/{label}/latentd-backend-collector")
    (output / binary["path"]).chmod(0o755)
    after = source(root)
    if before != after:
        raise ValueError("backend-build-source-changed")
    return {"source": before, "source_after": after, "inputs": retained, "executables": {"backend": binary},
            "command": argv, "process": receipt, "log": legacy.ref(directory / "build.log", output),
            "source_path": str(root), "target_path": str(target)}


def build_echo(root: Path, target: Path, output: Path, deadline: int) -> dict:
    directory = output / "builds" / "harness"
    directory.mkdir(parents=True)
    before = source(root)
    retained = inputs(root, "harness", output)
