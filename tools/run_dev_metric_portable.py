#!/usr/bin/env python3
"""Compare identical authored metric values, denial and exporter cleanup on Windows."""
...
def run(host: Path, inputs: Path, output: Path) -> dict:
    require(os.name == "nt" and not output.exists(), "actual-windows-and-new-metric-output-required")
    report = {"schemaVersion": "latent.dev.windows-metric-comparison.v1", "passed": False,
              "publisherAuthenticated": False, "cleanHost": False, "qualificationComplete": False,
              "cleanup": "unconfirmed"}
    with tempfile.TemporaryDirectory(prefix="lsf-metric-native-") as temporary:
        root = Path(temporary)
        require(not root.is_relative_to(Path(__file__).resolve().parents[1]), "native-metrics-outside-checkout")
        ...
        try:
            ...
            require("metrics" in node["identity"].get("fixtureConfiguration", {}), "actual-node-metric-fixture-required")
            ...
            report.update(passed=True, cleanup="owned-native-host-reaped-exporter-joined-queue-drained")
