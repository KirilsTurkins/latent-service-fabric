...
from tools.phase3_resource_profile import LIMITS, PROFILES, SCHEMA, canonical, digest, validate_receipt
...
def build(args):
    ...
    record = {"schemaVersion": "latent.phase3.resource-build.v1", "sourceRevision": args.revision,
              ...
              "command": command, "rustcVersion": compiler.stdout.decode("ascii").strip(),
              ...
              "startedUnixNanos": str(started), "finishedUnixNanos": str(time.time_ns()),
              "binaries": {"node": file_identity(target / "latentd"), "cli": file_identity(target / "latent")},
              "buildOutputDigest": digest({"stdout": outcome.stdout.decode("utf-8", "replace"),
                                            "stderr": outcome.stderr.decode("utf-8", "replace")})}
    write_receipt(args.record_build, record)
    print(json.dumps({"built": True, "identity": file_identity(args.record_build)}))
