...
def main(argv=None) -> int:
    ...
    parser.add_argument("--experiment", choices=("warm", "budget", "recovery", "ownership"), default="warm")
    ...
    parser.add_argument("--build-only", action="store_true", help="budget/recovery/ownership: retain collectors/inputs without measuring")
    parser.add_argument("--builds", type=Path, help="budget/recovery/ownership: collect beside a fresh copied revision-builds.json")
    ...
    try:
        ...
        if args.builds:
            if args.experiment not in ("budget", "recovery", "ownership") or any((args.build_only, args.output, args.control_ref, args.candidate_ref,
                                                   args.harness_ref, args.backend_build_output)):
                parser.error("--builds requires budget/recovery/ownership mode and cannot be combined with output/build/ref options")
        ...
        if args.build_only and args.experiment not in ("budget", "recovery", "ownership"):
            parser.error("--build-only is available only for budget/recovery/ownership experiments")
