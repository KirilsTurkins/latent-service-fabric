#!/usr/bin/env python3
"""Assemble exact-source Windows outputs on Linux using the existing SPDX owner."""
...
def main() -> int:
    ...
    require(record["sourceCommit"] == commit and not record["sourceDirty"] and record["hostAbi"] == HOST_ABI
            and record["protocol"] == PROTOCOL, "frontend-exact-source-required")
    require(file_digest(args.frontend / "dist/latent-dev/latent-dev.exe")[0] == record["frontendSha256"]
    ...
    require(portable_record == {"sourceCommit": commit, "sha256": file_digest(args.portable)[0],
                               "target": "windows-x86_64", "profile": "release"}, "portable-exact-source-required")
    ...
    shutil.copyfile(args.portable, payload / "bin/latent-portable-test-host.exe")
    ...
    metadata = json.loads(subprocess.check_output(["cargo", "metadata", "--locked", "--format-version", "1",
                         "--filter-platform", "x86_64-pc-windows-msvc"], cwd=ROOT, timeout=300))
    ...
    python_packages = [{"SPDXID": "SPDXRef-cpython", "name": "CPython", "versionInfo": python["python"],
        "filesAnalyzed": False, "downloadLocation": "https://www.python.org/ftp/python/3.13.5/python-3.13.5-amd64.exe",
    ...
    sbom["name"] = "LSF Windows developer frontend and portable test host"
    sbom["documentNamespace"] = "https://github.com/KirilsTurkins/latent-service-fabric/dev-windows-sbom/" + commit
    ...
    (payload / "build-provenance.json").write_bytes(encode({"schemaVersion": "latent.dev.build-provenance.v1",
        "sourceCommit": commit, "target": "windows-x86_64", "hostAbi": HOST_ABI, "protocol": PROTOCOL,
    ...
    value = assemble(payload, output / "candidate", commit=commit, version=version, target="windows-x86_64", epoch=epoch,
                     executables={"bin/latent-dev.exe", "bin/latent-portable-test-host.exe"})
