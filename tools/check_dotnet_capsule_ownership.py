...
def check(output, environment):
    ...
    for source, destination in (("tests/Ownership.csproj", "Ownership.csproj"), ("tests/Ownership.cs", "Checks.cs"),
            ("ownership/Owner.cs", "Owner.cs"), ("global.json", "global.json"), ("nuget.config", "nuget.config")):
    ...
    if b"C# ownership tests passed: 6" not in result:
        raise ValueError("incomplete C# ownership tests")
    report = {"status": "passed", "cases": 6, "commands": commands.records}
