...
180-second overall deadline; Go's cold runtime compilation allows 900.
...
def run(cli, node_binary, fixture, evidence, *, language="rust"):
    require(language in {"rust", "c", "go"}, "authoring-language")
    ...
    try:
        ...
        with owned_cancellation() as cancellation:
            with tempfile.TemporaryDirectory(prefix=f"lsf-{language}-authoring-node-") as temporary:
                ...
                seconds = 900 if language == "go" else 180
                invocation_millis = 120000 if language == "go" else 5000
