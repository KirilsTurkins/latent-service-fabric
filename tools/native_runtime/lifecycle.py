...
from .configuration import LOCAL, provision
from .layout import Layout, node_identity
from .verify import VerifiedRelease, manifest, version
...
def read_state(layout: Layout) -> dict | None:
    ...
    for selected, release in value["releases"].items():
        require(set(release) == {"publisher", "metadata"} and re.fullmatch(r"[0-9a-f]{64}", release["publisher"]),
...
def compatible(state: dict, selected: VerifiedRelease, upgrade: bool, profile: str) -> None:
    ...
    require(previous["publisher"] == selected.publisher, "publisher-key-rotation-needs-separate-reviewed-policy")
    ...
    require(expected in new["compatibility"]["upgradeFrom"] and new["engine"] == old["engine"],
...
def install(layout: Layout, release: VerifiedRelease, *, profile: str, policy: Path | None = None,
            ...
            resume: bool = False, acknowledge: bool = False) -> dict:
    ...
    with ExitStack() as stack:
        ...
        if layout.transaction.exists():
            ...
            require(pending.get("schemaVersion") == "latent.native-transaction.v1"
                    ...
                    and pending.get("profile") == profile, "resume-requires-exact-original-release-and-profile")
        else:
        ...
        if os.path.lexists(destination):
            ...
            with files.directory(layout.prefix, {0, controller[0]}) as parent, files.directory(releases, {0, controller[0]}) as target:
                ...
                os.fsync(target)
                os.fsync(parent)
        provision(layout, identity, profile, release.metadata, policy, port, pending["initial"])
