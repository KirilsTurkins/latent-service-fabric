//! Inline managed trigger auditing; this module starts no control worker.
mod mapping;
mod recovery;

pub use recovery::reconcile_trigger_audit;
...
use latent_control_store::{
    http_routes::{
        PreparedTriggerOperation, TriggerOperationCommit, TriggerOperationReceipt, TriggerRead,
    },
...
pub struct ManagedTriggerAudit {
...
impl ManagedTriggerAudit {
    ...
    #[must_use]
    pub fn matches(&self, actual: &TriggerOperationCommit) -> bool {
    ...
    pub fn commit(
        ...
        expires: Instant,
    ) -> crate::Result<TriggerRead<TriggerOperationCommit>> {
        check_deadline(expires)?;
        if self.started
        ...
        self.started = true;
        repository.commit_trigger_operation(prepared)
    }
    ...
    pub async fn finish(
        mut self,
        repository: &DirectoryDeploymentRepository,
        actual: Option<&TriggerOperationCommit>,
        expires: Instant,
    ) -> ReleaseAuditAck {
...
fn invalid() -> latent_core::PlatformError {
    crate::error(
        ...
        "trigger-audit-identity-mismatch",
    )
}
