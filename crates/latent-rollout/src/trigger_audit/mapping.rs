...
use latent_control_store::http_routes::{TriggerOperationAction, TriggerOperationReceipt};
...
fn identities(r: &TriggerOperationReceipt) -> AuditIdentities {
    AuditIdentities {
        trigger: Some(r.trigger_id.clone()),
        trigger_generation: Some(r.object_generation),
        publication: Some(r.publication.id.clone()),
        component: Some(r.component.clone()),
        deployment: Some(DeploymentId(r.deployment_id.clone())),
        deployment_generation: Some(r.deployment_generation),
        revision: Some(RevisionId(r.revision.clone())),
...
pub(super) fn attempt(
    ...
    Ok(AuditOperationAttempt {
        ...
        expected_deployment_generation: Some(r.deployment_generation),
