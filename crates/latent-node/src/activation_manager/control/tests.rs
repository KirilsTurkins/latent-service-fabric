...
async fn system_deadline_owns_one_sleep_and_never_completes_before_expiry() {
    ...
    assert!(poll(waiting.as_mut()).is_pending());
    assert_eq!(clock.observer.snapshot().armed, 1);
    ...
    assert_eq!((observed.live, observed.rechecks), (0, 1));
}
