...
impl Buffer {
    fn new(charged: &Arc<AtomicUsize>) -> Self {
        ...
        Self { bytes: Some(vec![0; 16]), charged: Arc::clone(charged) }
    }
}

impl Drop for Buffer {
    fn drop(&mut self) {
        assert_eq!(self.charged.load(Ordering::SeqCst), 16, "premature buffer refund");
...
fn missing_readiness_premature_retirement_and_illegal_transitions_fail() {
    ...
    assert_eq!(rendezvous.blocked(id, Stage::Requested), Err(CoordinationError::MissingReadiness));
    assert_eq!(rendezvous.require_retired(id), Err(CoordinationError::WrongStage));
    assert_eq!(owner.commit(Stage::Retired), Err(CoordinationError::InvalidTransition));
...
fn historical_arrival_abandoned_pause_and_recycled_registration_are_rejected() {
    ...
    drop(first);
    assert_eq!(rendezvous.blocked(id, Stage::Queued), Err(CoordinationError::MissingReadiness));
    let mut second = Box::pin(owner.pause());
    probe.pending(second.as_mut());
    assert_eq!(rendezvous.release(stale_pause), Err(CoordinationError::StaleRegistration));
    ...
    assert_eq!(rendezvous.snapshot(id), Err(CoordinationError::StaleRegistration));
    drop(next);
...
fn bounded_capacity_and_owner_unwind_do_not_leak() {
    ...
    assert!(matches!(rendezvous.track(()), Err(CoordinationError::Capacity)));
...
async fn cancellation_script() {
    with_watchdog(WATCHDOG, async {
        for panic_after_release in [false, true] {
            ...
            assert_eq!(rendezvous.require_retired(id), Err(CoordinationError::WrongStage));
            if panic_after_release { rendezvous.release(ticket).unwrap(); } else { task.abort(); }
...
async fn cancellation_and_panic_current_thread() { cancellation_script().await; }

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn cancellation_and_panic_multi_thread() { cancellation_script().await; }

#[tokio::test(start_paused = true)]
async fn watchdog_expires_even_with_paused_tokio_time() {
    let result = tokio::spawn(with_watchdog(Duration::from_millis(1), std::future::pending::<()>())).await;
