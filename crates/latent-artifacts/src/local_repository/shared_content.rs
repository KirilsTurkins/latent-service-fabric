...
impl SharedContent {
    ...
    pub(super) fn preflight_existing(
        &self,
        id: &PublicationId,
        source: &Path,
        files: &[(String, u64)],
        ...
        for (name, expected_size) in files {
            validate_name(name)?;
            let (key, size) = file_key(&source.join(name), self.config)?;
            if size != *expected_size {
                return Err(corrupt("migration-source-file-changed"));
            }
            logical = logical.checked_add(size).ok_or_else(capacity)?;
            keys.push(key.clone());
            if let Some(old) = self.blobs.get(&key) {
                if old.bytes != size {
                    return Err(corrupt("content-blob-size-conflict"));
                ...
                unique.insert(key, size);
            }
        }
        keys.sort();
        if let Some(old) = self.publications.get(id) {
            return if old.keys == keys && old.bytes == logical {
                Ok(())
            } else {
                Err(corrupt("publication-content-reference-conflict"))
            };
        }
        if self
            .blobs
    ...
    /// Migration hashes with fixed scratch and links immutable original bytes;
    /// it does not materialize an entire package or copy its Wasm into memory.
    pub(super) fn link_existing(
        &mut self,
        original: &Path,
        destination: &Path,
    ) -> Result<(), PlatformError> {
        self.check()?;
        let (key, bytes) = file_key(original, self.config)?;
        let source = self.root.join(&key.as_str()[7..]);
        if source.exists() {
            let (actual, size) = file_key(&source, self.config)?;
            if actual != key
                || size != bytes
                || self.blobs.get(&key).is_none_or(|b| b.bytes != bytes)
            {
            ...
            let result = fs::hard_link(original, &source)
                .map_err(io_error)
        ...
        if destination.exists() {
            let (actual, size) = file_key(destination, self.config)?;
            if actual != key || size != bytes {
                return Err(corrupt("migration-staging-file-conflict"));
            }
            return Ok(());
        }
    ...
    /// Read-only capacity reservation for the complete migration destination.
    pub(super) fn plan_directory(
        &mut self,
        id: &PublicationId,
        source: &Path,
    ) -> Result<(), PlatformError> {
        let files = bounded_files(source, self.config)?;
        self.preflight_existing(id, source, &files)?;
        let mut keys = Vec::with_capacity(files.len());
        let mut logical = 0u64;
        for (name, size) in files {
            let (key, actual) = file_key(&source.join(name), self.config)?;
            if actual != size {
                return Err(corrupt("migration-source-file-changed"));
            }
            logical = logical.checked_add(size).ok_or_else(capacity)?;
        ...
        keys.sort();
        self.register_keys(id, keys, logical)
    }
