//! Shared protected-file checks for trusted node bootstrap and secret reloads.
...
pub fn execution_profile_marker(root: &Path, create: bool) -> Result<bool, PlatformError> {
...
mod platform {
    ...
    fn marker_failure() -> PlatformError {
        crate::invalid("securityProfile.markerProtection")
    }
    ...
    impl Snapshot {
        pub(super) fn from(metadata: &std::fs::Metadata) -> Self {
            Self {
                device: metadata.dev(),
                inode: metadata.ino(),
                mode: metadata.mode(),
    ...
    {
        let failure = || crate::invalid(field);
    ...
    pub(super) fn normal_components(path: &Path) -> Result<Vec<OsString>, ()> {
    ...
    pub(super) fn require_mode_only_permissions(file: &File) -> Result<(), ()> {
    ...
    /// leaf private.
    pub(super) fn validate_directory(directory: &File, uid: u32, gid: u32) -> Result<bool, ()> {
...
mod platform {
    ...
    pub(super) fn execution_profile_marker(
        ...
        Err(crate::invalid("securityProfile.markerPlatformUnsupported"))
    }
