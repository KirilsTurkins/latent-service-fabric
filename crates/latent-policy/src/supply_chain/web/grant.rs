use super::super::{denied, Inner, State};
use latent_artifacts::{
    web::{WebAdmissionBinding, WebAdmissionGrant},
    AdmissionGrant, AdmissionRecheck,
};
...
pub(in crate::supply_chain) struct Grant {
    pub owner: Arc<Inner>,
    pub binding: WebAdmissionBinding,
    pub epoch: u64,
...
impl Grant {
    pub fn check(&self, owner: &Arc<Inner>, state: &mut State) -> Result<(), PlatformError> {
        ...
        state.verifiers()?.1.check_web_current(&self.builder, now)?;
        if !state
            .policy
            .tenants
