...
use latent_node::{
    LocalActivationDependencies, LocalActivationJournalConfig, LocalActivationManager,
...
use latent_wire::invocation::{
    proto, AuthenticatedInvocationContext, InvocationLimits, InvocationServiceAdapter,
    InvocationServiceServices, LocalInvocationRuntime,
};
use tokio::sync::Notify;

pub type Adapter = InvocationServiceAdapter<LocalInvocationRuntime>;
...
impl Default for Clock {
    fn default() -> Self {
        Self(Mutex::new(ClockSample::system_now()))
    }
}
...
impl Harness {
    pub fn adapter(&self) -> Adapter {
        InvocationServiceAdapter::with_services(
            Arc::new(LocalInvocationRuntime::new(self.manager.clone())),
    ...
    pub fn with_observer(
        clock: Arc<Clock>,
        observer: Arc<dyn latent_telemetry::ActivationObserver>,
    ) -> Self {
        Self::with_services(1, 8, clock, Some(observer))
    }
...
pub async fn pending<F: Future + ?Sized>(handle: Pin<&mut F>) {
...
pub async fn finish<F: Future>(handle: F) -> F::Output {
    tokio::time::timeout(Duration::from_secs(5), handle)
        .await
        .expect("lifecycle watchdog")
}

pub fn request(id: &str) -> proto::InvokeRequest {
    let request = model::request(id);
    proto::InvokeRequest {
        activation_id: request.activation_id.map(|id| id.0),
        parent_activation_id: request.parent_activation_id.map(|id| id.0),
        root_activation_id: request.root_activation_id.map(|id| id.0),
        target: Some(proto::InvocationTarget {
            tenant: request.target.tenant.0,
            service: request.target.service.0,
            contract: request.target.contract.0,
            function: request.target.function.0,
            route: request.target.route,
        }),
        payload: request.input,
        media_type: request.input_media_type,
        deadline_unix_millis: request.deadline_unix_millis,
        priority: u32::from(request.priority),
        idempotency_key: request.idempotency_key.map(|id| id.0),
        budget: Some(latent_wire::invocation::budget_to_proto(&request.budget)),
        metadata: request.metadata.into_iter().collect(),
    }
}

pub fn authenticated<T>(message: T) -> tonic::Request<T> {
    AuthenticatedInvocationContext::new(model::request("context").principal).request(message)
}

pub fn scoped<T>(message: T, tenant: &str, administrator: bool) -> tonic::Request<T> {
    let mut principal = model::request("context").principal;
    principal.tenant = Some(TenantId(tenant.to_owned()));
    if administrator {
        principal.kind = latent_core::PrincipalKind::Administrator;
    }
    AuthenticatedInvocationContext::new(principal).request(message)
}

pub async fn status(adapter: &Adapter, id: &str) -> proto::ActivationStatus {
...
pub async fn cancel(adapter: &Adapter, id: &str) -> proto::CancelResponse {
    use latent_wire::invocation::InvocationService;
    finish(adapter.cancel(authenticated(proto::CancelRequest {
        activation_id: id.to_owned(),
