...
pub fn invocation_request_to_proto(request: &InvocationRequest) -> proto::InvokeRequest {
    proto::InvokeRequest {
        ...
        idempotency_key: request.idempotency_key.as_ref().map(|key| key.0.clone()),
...
/// Lossless conversion for trusted, already bounded domain values.
pub fn invocation_response_to_proto(
    response: &InvocationResponse,
) -> Result<proto::InvokeResponse, InvocationConversionError> {
    validate_response_shape(response)?;
    let (result, consumption) = outcome_to_proto(&response.outcome);
    let pin = response.receipt.resolved_revision.as_ref();
    Ok(proto::InvokeResponse {
...
pub fn invocation_response_from_proto(
    mut response: proto::InvokeResponse,
) -> Result<InvocationResponse, InvocationConversionError> {
    let resolved_revision = shape::pin_from_wire(&mut response)?;
        ...
        .ok_or_else(|| InvocationConversionError::new("invocation result is missing"))?
    {
        ...
        proto::invoke_response::Result::DeclaredError(error) => ActivationOutcome::DeclaredError {
...
/// Lossless conversion for trusted, already bounded domain values.
pub fn activation_status_to_proto(
    status: &ActivationStatus,
) -> Result<proto::ActivationStatus, InvocationConversionError> {
    validate_status_shape(status)?;
    Ok(status_to_proto(status))
}
...
pub fn activation_status_from_proto(
    ...
    let terminal_outcome = status
        ...
        .map(|outcome| match outcome {
            proto::activation_status::TerminalOutcome::Succeeded(summary) => Ok(
            ...
            proto::activation_status::TerminalOutcome::DeclaredError(error) => Ok(
                RetainedActivationOutcome::DeclaredError(declared_error_from_proto(error)),
            ),
            proto::activation_status::TerminalOutcome::PlatformFailure(error) => Ok(
                RetainedActivationOutcome::PlatformFailure(platform_error_from_proto(error)?),
            ),
        })
        .transpose()?;
