...
use latent_control_store::deployment_operations::DeploymentReadLease;
use std::{
    future::Future,
    pin::Pin,
    task::{Context, Poll},
...
#[derive(Clone)]
pub struct DeploymentResponseService<S> {
    inner: S,
}
impl<S> DeploymentResponseService<S> {
    pub(crate) fn new(inner: S) -> Self {
        Self { inner }
    }
}
impl<S: NamedService> NamedService for DeploymentResponseService<S> {
    const NAME: &'static str = S::NAME;
}
impl<S, B> Service<http::Request<B>> for DeploymentResponseService<S>
where
...
fn retain(mut response: http::Response<Body>) -> http::Response<Body> {
    let lease = response.extensions_mut().remove::<DeploymentReadLease>();
    response.map(|body| {
        Body::new(LeasedBody {
...
struct LeasedBody {
    ...
    lease: Option<DeploymentReadLease>,
}
impl LeasedBody {
    fn finish(&mut self) {
