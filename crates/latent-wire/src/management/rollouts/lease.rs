...
use latent_rollout::ResponseLease;
use std::{
    future::Future,
    pin::Pin,
    task::{Context, Poll},
...
#[derive(Clone)]
pub struct RolloutResponseService<S> {
    inner: S,
}
impl<S> RolloutResponseService<S> {
    pub(crate) fn new(inner: S) -> Self {
        Self { inner }
    }
}
impl<S: NamedService> NamedService for RolloutResponseService<S> {
    const NAME: &'static str = S::NAME;
}
impl<S, B> Service<http::Request<B>> for RolloutResponseService<S>
where
