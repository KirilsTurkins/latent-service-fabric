...
use latent_routing::{InvocationTarget, ResolvedRevision, RevisionAdmissionPolicy, RevisionPolicySource};
...
fn node_policy() -> NodeAdmissionPolicy {
    ...
    let tenant = TenantAdmissionPolicy {
        limits: limits(), maximum_payload_bytes: 1024, maximum_priority: 200,
        allowed_subjects: names(&["alice"]), allowed_principal_kinds: vec![PrincipalKind::User],
    ...
    NodeAdmissionPolicy {
        budget_ceiling: ceiling, limits: limits(),
        tenants: BTreeMap::from([(TenantId("tenant-a".to_owned()), tenant), (TenantId("tenant-b".to_owned()), second)]),
        trust_classes: BTreeMap::from([("sandbox".to_owned(), TrustClassPolicy {
            limits: limits(), allowed_cell_classes: names(&["tiny", "small", "standard", "large"]),
        })]),
        queue_classes: BTreeMap::from([
            ("normal".to_owned(), QueueClassPolicy { minimum_priority: 0, maximum_priority: 127, maximum_queued_activations: 32 }),
            ("urgent".to_owned(), QueueClassPolicy { minimum_priority: 128, maximum_priority: 255, maximum_queued_activations: 4 }),
        ]),
        cell_classes: [
            ("tiny", 65_536, 2), ("small", 262_144, 2), ("standard", 1_048_576, 1),
            ("large", 4_194_304, 1), ("extra-large", 8_388_608, 1),
        ].into_iter().map(|(name, maximum_memory_bytes, parallelism)| (name.to_owned(), CellClassPolicy {
            maximum_memory_bytes, parallelism, threading_models: vec![ThreadingModel::SingleThreaded],
            features: if name == "tiny" { BTreeSet::new() } else { names(&["simd"]) },
        })).collect(),
        maximum_payload_bytes: 2048, maximum_priority: 255, maximum_identifier_bytes: 1024,
        maximum_metadata_entries: 32, maximum_metadata_bytes: 4096,
        overload: OverloadPolicy { maximum_cpu_pressure_milli: 900, maximum_memory_pressure_milli: 900, maximum_sample_age_millis: 10_000 },
        deadline: DeadlinePolicy { estimated_service_time_millis: 100, minimum_execution_time_millis: 5, safety_margin_millis: 5 },
        architecture: "x86_64".to_owned(), region: Some("local".to_owned()), zone: Some("a".to_owned()),
    }
}
...
fn revision(tenant: &str) -> ResolvedRevision {
    ResolvedRevision {
        target: InvocationTarget {
            ...
            contract: ContractId("example:echo/api@1.0.0".to_owned()), function: FunctionId("echo".to_owned()), route: None,
        },
        revision: RevisionId("revision-a".to_owned()), release: ReleaseDigest(format!("sha256:{}", "a".repeat(64))),
        route_generation: RouteGeneration(1), attributes: Metadata::new(),
    }
}
...
fn revision_policy() -> RevisionAdmissionPolicy {
    ...
    RevisionAdmissionPolicy {
        ...
        execution: ExecutionRequirements {
            ...
            state_model: StateModel::Stateless, resource_budget_ceiling: ceiling,
            host_call_depth_maximum: 8, component_call_depth_maximum: 8, snapshot_eligible: false, fusion_eligible: false,
        },
        placement: PlacementPolicy {
            trust_class: "sandbox".to_owned(), architectures: vec!["x86_64".to_owned()],
            regions: vec!["local".to_owned()], zones: vec!["a".to_owned()], required_features: vec![],
        },
    }
}
...
impl RevisionPolicySource for Source {
    fn admission_policy(&self, supplied: &ResolvedRevision) -> Result<RevisionAdmissionPolicy, PlatformError> {
        self.calls.fetch_add(1, Ordering::Relaxed);
        let mut supplied = supplied.clone();
        supplied.attributes.clear();
        if !self.available.load(Ordering::Relaxed) || ![revision("tenant-a"), revision("tenant-b")].contains(&supplied) {
            return Err(PlatformError { code: Code::RouteUnavailable, message: "other-tenant-secret".to_owned(), retryable: false, details: vec![] });
        }
...
impl Harness {
    fn new(policy: NodeAdmissionPolicy, revision_policy: RevisionAdmissionPolicy) -> Self {
        ...
        let source = Arc::new(Source { policy: revision_policy, available: AtomicBool::new(true), calls: AtomicUsize::new(0) });
        let load = Arc::new(NodeLoadState::new(NodeLoadSnapshot {
            accepting: true, cpu_pressure_milli: 0, memory_pressure_milli: 0,
            queue_delay_millis: 0, observed_at: sample.monotonic(),
        }).unwrap());
        let quotas = LocalQuotaProvider::new(policy).unwrap();
        let controller = LocalAdmissionController::new(source.clone(), quotas.clone(), load.clone());
        Self { controller, quotas, source, load, sample }
    }

    fn standard() -> Self { Self::new(node_policy(), revision_policy()) }

    fn request(&self, id: &str) -> AdmissionRequest {
        AdmissionRequest {
            activation_id: ActivationId(id.to_owned()),
            principal: InvocationPrincipal { subject: "alice".to_owned(), kind: PrincipalKind::User,
                tenant: Some(TenantId("tenant-a".to_owned())), service: None, claims: Metadata::new() },
            revision: revision("tenant-a"), requested_budget: budget(), deadline_unix_millis: None,
            payload_bytes: 10, priority: 10, attributes: Metadata::new(),
        }
    }
    ...
    fn assert_empty(&self) {
        ...
        for (tenant, policy) in &self.quotas.policy().tenants {
            ...
            assert_eq!(snapshot.remaining_cpu_fuel, policy.limits.maximum_reserved_cpu_fuel);
            assert_eq!(snapshot.remaining_memory_bytes, policy.limits.maximum_reserved_memory_bytes);
            assert_eq!(snapshot.reset_at_unix_millis, None);
        }
    }
}

fn detail(error: &PlatformError, name: &str) -> String { error.details[0].fields[name].clone() }
    ...
    for cpu in [0, 1, 100, 10_001, u64::MAX] {
        for memory in [0, 1, 65_536, 65_537, 262_144] {
            ...
            if cpu == 0 || memory == 0 {
                ...
                assert_eq!(permit.obligations().cell_class, if memory <= 65_536 { "tiny" } else { "small" });
            }
            h.assert_empty();
        }
    }
...
fn expired_zero_and_omitted_deadlines_have_distinct_semantics() {
    ...
    for deadline in [Some(0), Some(9999), Some(10_000)] {
        ...
        assert_eq!(h.controller.admit_at(request, h.sample).unwrap_err().code, Code::DeadlineExceeded);
        h.assert_empty();
    }
...
fn deadline_feasibility_uses_atomically_reserved_backlog_and_observed_delay() {
    ...
    let error = h.controller.admit_at(request.clone(), h.sample).unwrap_err();
...
fn queue_estimate_arithmetic_overflow_fails_closed() {
    ...
    let first = h.admit("a").unwrap();
    assert_eq!(detail(&h.admit("b").unwrap_err(), "reason"), "queue-estimate-overflow");
    assert_eq!(h.quotas.usage().unwrap().active_activations, 1);
    drop(first);
    h.assert_empty();
}
    ...
    for scope in ["node", "tenant", "trust-class"] {
        for dimension in ["concurrency", "queue", "cpu-fuel", "memory-bytes"] {
            ...
            let limit = match scope {
                ...
                "tenant" => &mut node.tenants.get_mut(&TenantId("tenant-a".to_owned())).unwrap().limits,
...
fn queue_handoff_returns_only_queue_capacity_until_terminal_cleanup() {
    ...
    let second = h.admit("b").unwrap();
    assert_eq!(detail(&h.admit("c").unwrap_err(), "dimension"), "concurrency");
    drop((running, second));
    h.assert_empty();
}
...
fn expired_handoff_consumes_and_releases_the_permit() {
    ...
    assert!(permit.ensure_schedulable_at(original_deadline - Duration::from_nanos(1)).is_ok());
    assert_eq!(permit.start_execution_at(original_deadline).unwrap_err().code, Code::DeadlineExceeded);
    h.assert_empty();
}
...
fn queue_class_capacity_and_priority_authorization_are_independent() {
    let mut node = node_policy();
    node.queue_classes.get_mut("normal").unwrap().maximum_queued_activations = 1;
    let h = Harness::new(node, revision_policy());
    let normal = h.admit("normal").unwrap();
    assert_eq!(detail(&h.admit("overflow").unwrap_err(), "scope"), "queue-class");
    ...
    assert_eq!(detail(&h.controller.admit_at(forbidden, h.sample).unwrap_err(), "dimension"), "priority");
    drop((normal, urgent));
    h.assert_empty();
}
...
fn payload_limits_are_exact_and_independent_for_node_and_tenant() {
    for (node_limit, tenant_limit, expected_scope) in [(9, 1024, "node"), (2048, 9, "tenant")] {
        ...
        node.tenants.get_mut(&TenantId("tenant-a".to_owned())).unwrap().maximum_payload_bytes = tenant_limit;
        let h = Harness::new(node, revision_policy());
        assert_eq!(detail(&h.admit("too-large").unwrap_err(), "scope"), expected_scope);
...
fn anonymous_forged_cross_tenant_and_unconfigured_principals_are_denied_before_lookup() {
    ...
    for mutate in mutations {
        ...
        mutate(&mut request);
        request.principal.claims.insert("role".to_owned(), "administrator".to_owned());
        assert_eq!(h.controller.admit_at(request, h.sample).unwrap_err().code, Code::PermissionDenied);
...
fn service_principals_require_a_valid_service_identity() {
    let mut node = node_policy();
    node.tenants.get_mut(&TenantId("tenant-a".to_owned())).unwrap().allowed_principal_kinds = vec![PrincipalKind::Service];
    let h = Harness::new(node, revision_policy());
    let mut request = h.request("service");
    request.principal.kind = PrincipalKind::Service;
    assert_eq!(h.controller.admit_at(request.clone(), h.sample).unwrap_err().code, Code::PermissionDenied);
...
fn exact_revision_release_generation_and_endpoint_must_exist() {
    ...
    assert_eq!(h.admit("missing").unwrap_err().code, Code::AdmissionRejected);
    h.assert_empty();
}
...
fn attributes_cannot_replace_catalog_policy_or_escape_in_error_details() {
    ...
    request.attributes.insert("trust_class".to_owned(), "administrator-secret".to_owned());
    request.revision.attributes.insert("lsf.deployment".to_owned(), "forged-resource-ceiling".to_owned());
    let permit = h.controller.admit_at(request, h.sample).unwrap();
    assert_eq!(permit.obligations().trust_class, "sandbox");
    assert!(permit.revision().attributes.is_empty());
    drop(permit);
    let mut request = h.request("denied");
    request.principal.claims.insert("secret".to_owned(), "private-token".to_owned());
...
fn metadata_identifier_and_empty_revision_boundaries_are_checked() {
    ...
    for field in ["activation", "revision", "function"] {
        ...
        assert_eq!(h.controller.admit_at(request, h.sample).unwrap_err().code, Code::InvalidArgument);
    }
    let mut request = h.request("metadata");
    request.principal.claims.insert("key".to_owned(), "x".repeat(4096));
    assert_eq!(detail(&h.controller.admit_at(request, h.sample).unwrap_err(), "dimension"), "metadata");
    let mut request = h.request("generation");
    request.revision.route_generation = RouteGeneration(0);
    assert_eq!(h.controller.admit_at(request, h.sample).unwrap_err().code, Code::AdmissionRejected);
    h.assert_empty();
}
...
fn incompatible_backends_state_threading_features_and_placement_are_independent() {
    let mutations: [(fn(&mut RevisionAdmissionPolicy), &str); 6] = [
        (|p| p.execution.backend = ExecutionBackendKind::Container, "backend"),
        (|p| p.execution.state_model = StateModel::Entity, "state-model"),
        (|p| p.execution.threading = ThreadingModel::Cooperative, "threading"),
        (|p| p.placement.required_features = vec!["missing".to_owned()], "required-features"),
        (|p| p.placement.architectures = vec!["aarch64".to_owned()], "architecture"),
    ...
    for (mutate, dimension) in mutations {
        ...
        assert_eq!(detail(&h.admit("unsupported").unwrap_err(), "dimension"), dimension);
        h.assert_empty();
    }
}
...
fn extra_large_requires_explicit_tenant_and_trust_permission() {
    for permit_tenant in [false, true] {
        for permit_trust in [false, true] {
            let mut node = node_policy();
            if permit_tenant { node.tenants.get_mut(&TenantId("tenant-a".to_owned())).unwrap().allowed_cell_classes.insert("extra-large".to_owned()); }
            if permit_trust { node.trust_classes.get_mut("sandbox").unwrap().allowed_cell_classes.insert("extra-large".to_owned()); }
            let h = Harness::new(node, revision_policy());
...
fn trust_class_permission_cannot_be_granted_by_request_metadata() {
    ...
    request.attributes.insert("trust_class".to_owned(), "sandbox".to_owned());
    assert_eq!(detail(&h.controller.admit_at(request, h.sample).unwrap_err(), "dimension"), "trust-class");
    h.assert_empty();
}
...
fn overload_pressure_thresholds_are_exact_and_independent() {
    for cpu in [false, true] {
        let h = Harness::standard();
        let mut load = h.load.snapshot().unwrap();
        if cpu { load.cpu_pressure_milli = 899; } else { load.memory_pressure_milli = 899; }
        h.load.publish(load).unwrap();
        drop(h.admit("below").unwrap());
        if cpu { load.cpu_pressure_milli = 900; } else { load.memory_pressure_milli = 900; }
        h.load.publish(load).unwrap();
        let error = h.admit("at-threshold").unwrap_err();
        assert_eq!(error.code, Code::ResourceExhausted);
        assert_eq!(detail(&error, "dimension"), if cpu { "cpu-pressure" } else { "memory-pressure" });
        h.assert_empty();
    }
}
...
fn stale_future_malformed_and_unavailable_load_samples_fail_closed() {
    ...
    assert_eq!(h.controller.admit_at(h.request("stale"), stale).unwrap_err().code, Code::Unavailable);
    let future = ClockSample::new(9999, h.sample.monotonic() - Duration::from_millis(1));
    assert_eq!(h.controller.admit_at(h.request("future"), future).unwrap_err().code, Code::Unavailable);
    let mut load = h.load.snapshot().unwrap();
...
fn duplicate_activation_ids_do_not_replace_or_refund_live_reservations() {
    ...
    assert_eq!(h.controller.admit_at(request, h.sample).unwrap_err().code, Code::AlreadyExists);
    assert_eq!(h.quotas.usage().unwrap().active_activations, 1);
    drop(original);
...
fn tenant_counters_are_isolated_while_node_and_trust_limits_are_shared() {
    ...
    for tenant in ["tenant-a", "tenant-b"] {
        assert_eq!(h.quotas.snapshot_now(&TenantId(tenant.to_owned())).unwrap().active_activations, 1);
    }
    assert_eq!(h.quotas.usage().unwrap().active_activations, 2);
    drop(first);
...
fn all_terminal_outcomes_release_reserved_capacity_after_accounting() {
    let h = Harness::standard();
    for outcome in ["success", "declared-error", "trap", "deadline", "cancelled", "platform-failure", "cleanup-failure"] {
        ...
        std::thread::scope(|scope| {
            for index in 0..64 {
                ...
                scope.spawn(move || {
                    ...
                    if permit.is_some() { winners.fetch_add(1, Ordering::SeqCst); }
                    admitted.wait();
                    release.wait();
                    drop(permit);
                });
...
fn controller_replacement_shares_existing_quota_reservations() {
    ...
    assert_eq!(detail(&next.admit_at(h.request("new"), h.sample).unwrap_err(), "dimension"), "concurrency");
    drop(original);
...
fn maximum_integer_reservations_cannot_wrap_or_double_refund() {
    ...
    for tenant in node.tenants.values_mut() { tenant.limits.maximum_reserved_cpu_fuel = u64::MAX; }
    node.trust_classes.get_mut("sandbox").unwrap().limits.maximum_reserved_cpu_fuel = u64::MAX;
    let mut policy = revision_policy();
    ...
    assert_eq!(detail(&h.admit("overflow").unwrap_err(), "dimension"), "cpu-fuel");
    drop(permit);
    h.assert_empty();
...
fn invalid_startup_policies_are_rejected_before_any_state_exists() {
    let mutations: [fn(&mut NodeAdmissionPolicy); 7] = [
        ...
        |p| p.cell_classes.get_mut("small").unwrap().maximum_memory_bytes = 1,
    ];
    for mutate in mutations {
