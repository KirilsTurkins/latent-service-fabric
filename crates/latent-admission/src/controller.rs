...
impl LocalAdmissionController {
    ...
    #[must_use]
    pub fn quotas(&self) -> LocalQuotaProvider {
        self.quotas.clone()
    }
    ...
    fn admit_observed_at(
        ...
        let policy = self
            .source
            .admission_policy(&request.revision)
            .map_err(|error| {
        ...
        let trust = node
            .trust_classes
            .get(&policy.placement.trust_class)
            .filter(|_| {
                tenant
                    .allowed_trust_classes
                    .contains(&policy.placement.trust_class)
            })
            .ok_or_else(|| {
                rejection(
        ...
        let class = select_class(
            ...
            node,
            tenant,
            &trust.allowed_cell_classes,
            grant.budget.memory_bytes,
        )?;
        let queue = node
            .queue_classes
            .iter()
            .find(|(_, queue)| {
                (queue.minimum_priority..=queue.maximum_priority).contains(&request.priority)
            })
...
fn validate_request<'a>(
    request: &AdmissionRequest,
    node: &'a NodeAdmissionPolicy,
) -> Result<&'a TenantAdmissionPolicy, PlatformError> {
    ...
    if principal.tenant.as_ref() != Some(&request.revision.target.tenant)
        ...
        || principal
            .service
            .as_ref()
            .is_some_and(|service| !valid_identifier(&service.0, node.maximum_identifier_bytes))
    {
        return Err(rejection(
            PlatformErrorCode::PermissionDenied,
            "request",
    ...
    let tenant = node
        .tenants
        .get(&request.revision.target.tenant)
        .filter(|tenant| {
            tenant.allowed_subjects.contains(&principal.subject)
                && tenant.allowed_principal_kinds.contains(&principal.kind)
        })
        .ok_or_else(|| {
            rejection(
                PlatformErrorCode::PermissionDenied,
    ...
    for id in [
        request.activation_id.0.as_str(),
    ...
    for (limit, scope) in [
        (node.maximum_payload_bytes, "node"),
        (tenant.maximum_payload_bytes, "tenant"),
    ] {
    ...
    for metadata in [
        ...
        for (key, value) in metadata {
            ...
            bytes = bytes
                ...
                .filter(|size| *size <= node.maximum_metadata_bytes)
                .ok_or_else(metadata_rejection)?;
        }
    }
    Ok(())
...
fn validate_revision_policy(
    ...
    for (supported, dimension) in [
        (
            policy.execution.backend == ExecutionBackendKind::WasmComponent,
            "backend",
        ),
        (
            policy.execution.state_model == StateModel::Stateless,
            "state-model",
        ),
        (
            policy.execution.host_call_depth_maximum > 0
                && policy.execution.component_call_depth_maximum > 0,
            "call-depth",
        ),
    ] {
        if !supported {
    ...
    for (allowed, value, dimension) in [
        (
            &policy.placement.architectures,
            Some(node.architecture.as_str()),
            "architecture",
        ),
        ...
        if !allowed.is_empty()
            && !value.is_some_and(|value| allowed.iter().any(|allowed| allowed == value))
        {
            return Err(rejection(
                PlatformErrorCode::AdmissionRejected,
                "node",
...
fn select_class(
    ...
    for (name, class) in &node.cell_classes {
        if class.maximum_memory_bytes < memory {
            continue;
        }
        memory_supported = true;
        if !class.threading_models.contains(&policy.execution.threading) {
            continue;
        }
        ...
        if !policy
            ...
            .required_features
            .iter()
            .all(|feature| class.features.contains(feature))
        {
            continue;
        }
...
fn validate_load(
    ...
    if now
        .checked_duration_since(load.observed_at)
    ...
    for (value, maximum, dimension) in [
        (
            load.cpu_pressure_milli,
            node.overload.maximum_cpu_pressure_milli,
            "cpu-pressure",
        ),
        (
            load.memory_pressure_milli,
            node.overload.maximum_memory_pressure_milli,
            "memory-pressure",
        ),
    ] {
        if value >= maximum {
