...
fn concurrent_tenant_trust_and_queue_class_reservations_are_linearized() {
    for scope_name in ["tenant", "trust-class", "queue-class", "deadline"] {
        ...
        match scope_name {
            "tenant" => node.tenants.get_mut(&TenantId("tenant-a".to_owned())).unwrap().limits.maximum_concurrent_activations = 3,
            "trust-class" => node.trust_classes.get_mut("sandbox").unwrap().limits.maximum_concurrent_activations = 3,
            "queue-class" => node.queue_classes.get_mut("normal").unwrap().maximum_queued_activations = 3,
        ...
        std::thread::scope(|scope| {
            for index in 0..32 {
                ...
                if scope_name == "deadline" { request.deadline_unix_millis = Some(10_120); }
                let sample = h.sample;
                let winners = &winners;
                scope.spawn(move || {
                    start.wait();
                    let result = controller.admit_at(request, sample);
                    if result.is_ok() { winners.fetch_add(1, Ordering::SeqCst); }
                    // All participants reach both barriers even if a result is
                    // incorrect, so a regression produces an assertion, not a hang.
                    counted.wait();
                    release.wait();
                    if let Err(error) = &result {
                        assert_eq!(detail(error, if scope_name == "deadline" { "dimension" } else { "scope" }), scope_name);
                    }
                    drop(result);
                });
            }
...
impl RevisionPolicySource for DelayedSource {
    fn admission_policy(&self, revision: &ResolvedRevision) -> Result<RevisionAdmissionPolicy, PlatformError> {
...
fn live_admission_resamples_time_after_policy_lookup_instead_of_granting_expired_work() {
    ...
    let h = Harness::new(node, revision_policy());
    let controller = h.controller.with_policy_source(Arc::new(DelayedSource { inner: h.source.clone() }));
...
fn final_timing_check_rejects_stale_observations_and_exact_expiry() {
    ...
    let grant = latent_core::EffectiveActivationBudget::admit_at(&budget(), &node.budget_ceiling, &node.budget_ceiling, None, sample).unwrap();
    ...
    assert_eq!(stale.validate(&node, &grant, 0, 1).unwrap_err().code, Code::Unavailable);
    let expired = ReservationTiming { clock: AdmissionClock::Fixed(grant.deadline.monotonic().unwrap()), ..stale };
    assert_eq!(expired.validate(&node, &grant, 0, 1).unwrap_err().code, Code::DeadlineExceeded);
}
...
fn metadata_limits_cover_all_three_maps_at_the_exact_boundary() {
    ...
    exact.principal.claims.insert("a".to_owned(), "x".to_owned());
    exact.attributes.insert("b".to_owned(), "y".to_owned());
    exact.revision.attributes.insert("c".to_owned(), "z".to_owned());
    drop(h.controller.admit_at(exact.clone(), h.sample).unwrap());
    let mut byte_overflow = exact.clone();
    byte_overflow.attributes.insert("b".to_owned(), "yy".to_owned());
    assert_eq!(detail(&h.controller.admit_at(byte_overflow, h.sample).unwrap_err(), "dimension"), "metadata");
    exact.attributes.insert("d".to_owned(), String::new());
    assert_eq!(detail(&h.controller.admit_at(exact, h.sample).unwrap_err(), "dimension"), "metadata");
    h.assert_empty();
}
...
fn unavailable_load_is_sanitized_and_old_samples_cannot_replace_newer_ones() {
    ...
    impl NodeLoadSource for FailedLoad {
        fn snapshot(&self) -> Result<NodeLoadSnapshot, PlatformError> {
            Err(PlatformError { code: Code::Internal, message: "private-pressure-source-token".to_owned(), retryable: false, details: vec![] })
        }
    }
    let h = Harness::standard();
    let controller = LocalAdmissionController::new(h.source.clone(), h.quotas.clone(), Arc::new(FailedLoad));
    let error = controller.admit_now(Harness::request("unavailable")).unwrap_err();
    ...
    older.observed_at = older.observed_at.checked_sub(Duration::from_nanos(1)).unwrap();
...
fn memory_without_a_compatible_configured_cell_is_rejected() {
    ...
    for tenant in node.tenants.values_mut() { tenant.allowed_cell_classes = names(&["tiny"]); }
    node.trust_classes.get_mut("sandbox").unwrap().allowed_cell_classes = names(&["tiny"]);
    let h = Harness::new(node, revision_policy());
...
fn admission_state_does_not_grow_with_rejected_service_cardinality() {
    ...
    if std::env::var(CHILD).as_deref() != Ok("1") {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", "tests::stress::admission_state_does_not_grow_with_rejected_service_cardinality", "--nocapture", "--test-threads=1"])
    ...
    let resources = || {
        ...
        let sockets = std::fs::read_dir("/proc/self/fd").unwrap().filter_map(Result::ok)
            .filter_map(|entry| std::fs::read_link(entry.path()).ok())
            .filter(|target| target.to_string_lossy().starts_with("socket:")).count();
        let children = std::fs::read_dir("/proc/self/task").unwrap().filter_map(Result::ok)
            .map(|entry| std::fs::read_to_string(entry.path().join("children")).unwrap().split_whitespace().count()).sum::<usize>();
