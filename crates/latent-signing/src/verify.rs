...
pub use proof::VerifiedPackageSignature;

use crate::{
    evidence::inspect_evidence, PackageSigningSubject, PublisherTrust, SignatureEvidenceRef,
    SignatureFailure, SignatureLimits, SignatureResult, TrustStateId,
};
use std::{
    fmt,
    sync::{
...
/// One bounded publisher trust owner. It performs synchronous bounded crypto,
...
impl PublisherVerifier {
    pub fn new(trust: PublisherTrust, limits: SignatureLimits, now: u64) -> SignatureResult<Self> {
        limits.validate()?;
        trust.fits(limits)?;
        trust.fresh(now)?;
        Ok(Self {
    ...
    pub fn check_current(&self, proof: &VerifiedPackageSignature, now: u64) -> SignatureResult<()> {
        self.observe(now)?;
        self.check_captured(proof, now)
    }
    ...
    pub fn replace_trust(
        &self,
        expected: &TrustStateId,
        next: PublisherTrust,
        now: u64,
    ) -> SignatureResult<TrustStateId> {
        self.observe(now)?;
...
impl fmt::Debug for PublisherVerifier {
