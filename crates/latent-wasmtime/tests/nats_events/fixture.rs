...
impl Fixture {
    ...
    pub async fn with_publication(
        ...
        write(&root.join("password"), b"lsf-public-nats-password");
        ...
        .await
        .unwrap();
        secrets.reload(0, specs(&config)).unwrap().await.unwrap();
        let credentials = [("tests", "publisher"), ("other", "other")]
            .into_iter()
            .map(|(tenant, user)| NatsCredential {
            ...
            .collect();
        let provider =
            NatsPublisher::install(pools.clone(), "events", 1, 0, config.clone(), credentials)
                .unwrap();
        install(
            &policies,
            &publication,
            ...
            required,
            latent_nats::NATS_PUBLISH_PROFILE,
        );
        ...
        let plan = broker
            .compile_invocation_plan(
                ...
                &[CapabilityBindingSpec {
                    ...
                    imported_operations: &["publish".into()],
                    policy_ids: &["p".into()],
    ...
    for (id, kind, value) in [
        (
            ...
            json!({"formatVersion":1,"tenant":"tests","rules":[{
                ...
                "operations":["publish"], "resources":{"kind":"events","subjects":["allowed","broker-denied","tenant-only"]},
...
pub fn specs(config: &NatsConfig) -> Vec<SecretSpec> {
    ["tests", "other"]
        .into_iter()
