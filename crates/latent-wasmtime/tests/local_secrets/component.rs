//! Actual sync-WIT guest: return a synthetic value/version marker or typed error.
use wasm_encoder::*;
#[path = "../../../latent-packaging/tests/fixtures/host.rs"]
mod host;
pub const CONTRACT: &str = "tests:local-secrets/api@1.0.0";
pub const CAP: &str = latent_capabilities::broker::secrets::SECRETS_CAPABILITY;
...
pub fn bytes() -> Vec<u8> {
    ...
    let mut instances = InstanceSection::new();
    instances.export_items([("read", ExportKind::Func, 1)]);
    instances.instantiate(
        1,
        [
            ("secret", ModuleArg::Instance(1)),
...
fn memory() -> Module {
    ...
    for (index, name) in ["allowed", "denied", "opaque", "tenant-only", "expired"]
        .iter()
        .enumerate()
    {
        data.active(
...
fn caller() -> Module {
    ...
    imports.import("secret", "read", EntityType::Function(0));
    imports.import(
        "memory",
        "memory",
    ...
    for (which, length) in [(1, 6), (2, 6), (3, 11), (4, 7)] {
        for i in [
