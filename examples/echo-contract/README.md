...
The artifact is locally trusted because it is produced from the current checkout with the committed `Cargo.lock`, the selected Rust toolchain, a self-contained `wasm32-unknown-unknown` core compiled with the exact workspace `wit-bindgen` requirement from `Cargo.toml`, and explicit componentization with the selected `wasm-tools`. It is not signed and is not an OCI artifact. `capsule.json` and `build.json` record the concrete versions used by that checkout.
