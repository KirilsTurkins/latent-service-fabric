...
In current [CI](https://github.com/KirilsTurkins/latent-service-fabric/blob/development/.github/workflows/ci.yml), the contracts job runs `tools/validate_contracts.sh`, then `bash tools/run_phase0_outcome_matrix.sh` against the echo and containment fixtures that job already built. The matrix keeps the executable outcome/recovery coverage on pull requests without rerunning the standalone baseline collector. The separate [Phase 0 runtime regression workflow](https://github.com/KirilsTurkins/latent-service-fabric/blob/development/.github/workflows/phase0-regression.yml) is manual-only; it collects the deterministic baseline and then runs that matrix. Its results remain regression evidence, not a fresh full authorization or performance claim.

The [Phase 2 completion report](https://github.com/KirilsTurkins/latent-service-fabric/blob/development/docs/phase-2-completion.md) records the gate decision and exact evidence. Phase 3 is the next planned workstream: its [41 tickets](https://github.com/KirilsTurkins/latent-service-fabric/issues/201) are decomposed capabilities with dependencies and resource/security acceptance. Implement against each ticket's prerequisites and current development; planning and issue creation are not implementation delivery.
